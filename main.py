...
# Dichiarare tree come variabile globale, sono Leo
tree = None
...
def on_tree_double_click(event):
    ...
    directory_path = tree.item(item, "values")[
        0
    ]  # Ottieni il valore della colonna "Directory"
...
def select_directory():
    ...
    result = messagebox.askquestion(
        "Scansione", "Scansionare anche i file?", icon="question"
    )
    if result == "yes":
        scan_files = True
    else:
    ...
    if directory_path:
        ...
        progress_bar.start()
        t = threading.Thread(
            target=scan_and_save_subdirectories, args=(directory_path, on_scan_complete)
        )
        t.start()
...
def search_directory(event=None):
    ...
    if show_files_var.get():
        ...
        query = (
            "SELECT directory FROM directories WHERE "
            + search_query
            + " AND is_file = 0"
        )
    ...
    ]  # Estrai i risultati correttamente
...
def clear_database():
    ...
    confirmation = messagebox.askquestion(
        "Conferma", "Sei sicuro di voler svuotare il database?", icon="warning"
    )
...
def scan_and_save_subdirectories(directory, callback):
    ...
    try:
        ...
        messagebox.showerror(
            "Errore", f"Errore durante il calcolo di total_files: {str(e)}"
        )
    ...
    for progress, (root, dirs, files) in enumerate(os.walk(directory)):
        ...
        for dir_name in dirs:
            ...
            dir_path = os.path.join(root, dir_name).replace(
                "\\", "/"
            )  # Sostituisci le barre rovesciate
            cursor.execute(
                "SELECT directory FROM directories WHERE directory = ? AND is_file = 0",
                (dir_path,),
            )
            existing_dir = cursor.fetchone()
            if not existing_dir:
                cursor.execute(
                    "INSERT INTO directories (directory, is_file) VALUES (?, ?)",
                    (dir_path, 0),
                )
        ...
        if scan_files:
            for file_name in files:
                ...
                cursor.execute(
                    "SELECT directory FROM directories WHERE directory = ? AND is_file = 1",
                    (file_path,),
                )
                ...
                if not existing_file:
                    ...
                    num_items += 1
                    status_label.config(
                        text=f"Scansione in corso...{directory}{num_items}"
                    )
...
def scan_directories_from_queue():
    while not directory_queue.empty():
        ...
        t = threading.Thread(
            target=scan_and_save_subdirectories, args=(directory, on_scan_complete)
        )
        t.start()
...
def start_auto_scan():
    ...
    if result == "yes":
        ...
        scan_files = False
    auto_scan_button["state"] = (
        "disabled"  # Disabilita il pulsante durante le scansioni
    )
...
def delete_selected_item():
    ...
    if selected_items:
        ...
        messagebox.showwarning(
            "Nessuna riga selezionata", "Seleziona una riga prima di eliminare."
        )
...
def open_configurations_window():
    ...
    configurations_scrollbar = ttk.Scrollbar(
        configurations_window, orient="vertical", command=configurations_tree.yview
    )
    ...
    add_button = tk.Button(
        configurations_window, text="Aggiungi", command=add_configuration
    )
    add_button.pack(side="left", padx=5, pady=5)

    # Aggiungi il pulsante "Elimina"
    delete_button = tk.Button(
        configurations_window, text="Elimina", command=delete_configuration
    )
...
def delete_configuration():
    ...
    if selected_item:
        ...
        updated_data = [
            entry for entry in data if entry["directory"] != directory_to_delete
        ]
...
def add_configuration():
    ...
    save_button = tk.Button(
        add_window,
        text="Salva",
        command=lambda: save_new_configuration(add_window, new_directory_entry.get()),
    )
    save_button.pack()
...
select_button = tk.Button(
    button_frame, text="Seleziona Directory", command=select_directory
)
...
show_all_button = tk.Button(
    button_frame,
    text="Mostra Tutti",
    command=lambda: show_all_items(show_files_var.get()),
)
show_all_button.pack(side="left", padx=5, pady=5)

clear_database_button = tk.Button(
    button_frame, text="Svuota Database", command=clear_database
)
...
auto_scan_button = tk.Button(
    button_frame, text="Scansione Automatica", command=start_auto_scan
)
auto_scan_button.pack(side="left", padx=5, pady=5)

view_configurations_button = tk.Button(
    button_frame, text="Visualizza Configurazioni", command=open_configurations_window
)
...
show_files_checkbutton = tk.Checkbutton(
    button_frame, text="Show Files", variable=show_files_var
)
...
)  # Associa il menu contestuale al tasto destro del mouse
